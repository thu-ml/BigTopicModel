...
static void solve_l2r_l1l2_svc(
	...
	double Cp, double Cn, double *saved_alpha, int solver_type)
{
    int l = prob->l; // doc num
    int w_size = prob->n; // topic num
	int i, s, iter = 0;
	double C, d, G;
	double *QD = new double[l];
	int max_iter = 1000;
...
static void train_one(const problem *prob, const parameter *param, double *w, double Cp, double Cn, double* saved_alpha)
{
	...
	switch(param->solver_type)
	{
		...
		case L2R_L2LOSS_SVC_DUAL:
			solve_l2r_l1l2_svc(prob, w, eps, Cp, Cn, saved_alpha, L2R_L2LOSS_SVC_DUAL);
			break;
		case L2R_L1LOSS_SVC_DUAL:
			solve_l2r_l1l2_svc(prob, w, eps, Cp, Cn, saved_alpha, L2R_L1LOSS_SVC_DUAL);
			break;
		case L1R_L2LOSS_SVC:
		{
			problem prob_col;
...
model* train(const problem *prob, const parameter *param)
{
	...
	if(check_regression_model(model_))
	{
		...
		model_->nr_class = 2;
		model_->label = NULL;
		train_one(prob, param, model_->w, 0, 0, empty);
	}
	else
	{
		int nr_class;
		...
		model_ -> doc_mapping = Malloc(int,l);
        ...
        // added by Jiarui Lu
        model_ -> saved_alpha = Malloc(double*, nr_class);
        for (int i = 0; i < nr_class; i ++)
            model_ -> saved_alpha[i] = Malloc(double, l);
...
void free_model_content(struct model *model_ptr)
{
    ...
    // added by Jiarui Lu
    if(model_ptr -> saved_alpha != NULL)
        free(model_ptr -> saved_alpha);
		if (model_ptr -> doc_mapping != NULL)
				free(model_ptr -> doc_mapping);
}
